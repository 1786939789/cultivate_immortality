...
- 前端首页同步使用 `GET /api/home`；该接口通过 `homeIncrementalRepository.mjs` 只读取玩家、metrics 和首页 section，不得调用完整 `loadStateFromMysql()`。action 成功后优先合并 patch，再请求轻量 home projection。
