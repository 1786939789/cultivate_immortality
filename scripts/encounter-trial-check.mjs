...
assert.equal(daoTrialSeals.length, 1024, "问道秘境应提供一千零二十四道印");
assert.equal(daoTrialLaws.length, 256, "问道秘境应提供二百五十六项问道法则");
assert.equal(new Set(daoTrialSeals.map((seal) => seal.id)).size, 1024, "道印 ID 必须唯一");
...
assert.match(state.daoTrial.history[0].startedAt, /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/, "问道记录应保存入场时间");
assert.match(state.daoTrial.history[0].endedAt, /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/, "问道记录应保存结算时间");
assert.equal(getPublicState(state).daoTrial.history[0].endedAt, state.daoTrial.history[0].endedAt, "公开问道档案应包含结算时间");
